...
import TestTube from '@/assets/model/testtube_iodine'
import Dropper from '@/assets/model/Dropper_06'
...
export const ExpContext = React.createContext({
  state: initialState,
  setState: (prev: React.SetStateAction<typeof initialState>) => {},
})
  ...
  function testTubeHandler(testtube: number) {
    setState((e) => {
      ...
      setTimeout(() => {
        setState((e) => ({
          ...e,
          color: testtube == 0 ? ['black', e.color[1]] : e.color,
        }))
      }, 6000)
      setTimeout(() => {
        setState((e) => ({
          ...e,
          color: testtube == 1 ? [e.color[0], 'yellow'] : e.color,
        }))
      }, 7000)
